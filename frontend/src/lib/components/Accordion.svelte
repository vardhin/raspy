<script lang="ts">
	// A single accordion section: a clickable header snippet and a collapsible body
	// snippet. Token-only styling, so it re-skins with any theme.
	//
	...
	const textEl = $derived(
		bodyEl?.querySelector<HTMLElement>('[data-ac-text]') ?? bodyEl
	);
...
<div class="accordion" class:open class:measured>
	<button
		class="ac-header"
		type="button"
		aria-expanded={open}
